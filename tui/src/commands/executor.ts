...
const SCREEN_ALIASES: Record<string, ScreenTab> = {
...
export async function executeCommand(
  ...
  try {
    ...
    if (cmd === "go" && parts[1]) {
      ...
      if (tab) return { output: `Switched to ${parts[1]}`, navigate: tab };
    ...
    if (cmd === "dev") {
      ...
      if (!action || !project) return { output: "Usage: dev start|stop|restart <project>", error: true };
      if (action === "start") {
    ...
    if (cmd === "kill") {
      ...
      if (isNaN(pid)) return { output: "Usage: kill <pid> | kill all", error: true };
    ...
    if (cmd === "task") {
      ...
      if (!project || !text) return { output: "Usage: task <project> <text>", error: true };
    ...
    if (cmd === "config") {
      ...
      if (action === "set") {
        ...
        if (!key || !value) return { output: "Usage: config set <key> <value>", error: true };
        await updateConfig(key, value);
        return { output: `Config set: ${key} = ${value}` };
      } else if (action === "delete" || action === "del" || action === "rm") {
        const key = parts[2];
        if (!key) return { output: "Usage: config delete <key>", error: true };
    ...
    if (cmd === "projects" || cmd === "ls") {
      ...
      const lines = projects.map(
        (p) => `  ${p.name.padEnd(22)} ${p.devStatus?.status ?? "—"}`,
      );
    ...
    if (cmd === "sessions" || cmd === "ps") {
      ...
      if (data.totalProcesses === 0) return { output: "No active Claude sessions" };
      const lines: string[] = [`${data.totalProcesses} active sessions:`];
...
async function streamClaude(
  ...
  if (!reader) return { output: "No response body", error: true };
